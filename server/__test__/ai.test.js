    ...
    await userCollection.insertMany([
        {
            ...
            role: 'user',
            deviceId: 'ExponentPushToken[yXlQ1dNg7ONO1Q6quG26PP]'
        },
        {
            fullName: 'udinaja1',
...
describe('GET /send-notification', () => {
    describe('GET /send-notification - succeed', () => {
        it('should be return an message from gemini ai', async () => {
            const response = await request(app)
                .get('/send-notification')
                .set('Authorization', `Bearer ${access_token_admin}`)

            expect(response.status).toBe(200)
            expect(response.body).toHaveProperty('notification', expect.any(String))
        })
    })
    describe('GET /send-notification - failed', () => {
